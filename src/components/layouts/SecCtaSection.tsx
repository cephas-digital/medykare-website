const SecCtaSection = () => {
  return (
    <section className="py-16 bg-[#F8FEFF] lg:px-16 md:px-10 px-5  mt-16">
      <div
        data-aos="fade-left"
        data-aos-duration="1500"
        className=" mx-auto px-6 text-left "
      >
        <h2 className="text-2xl lg:text-3xl  font-medium text-[#262626] mb-6 leading-tight">
          Are you ready to embark on a transformative journey towards better{" "}
          <br />
          health and wellness?
        </h2>
        <p className="text-[#333333] font-Outfit text-base lg:text-lg font-light leading-relaxed">
          Join us at MedyKare today and let us guide you on the path to a
          healthier and happier you.
        </p>
      </div>
    </section>
  );
};

export default SecCtaSection;
